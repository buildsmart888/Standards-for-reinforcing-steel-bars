...
module GoRebar
  module StirrupSystem90
    ...
    def self.show_dialog
      ...
      inputs = UI.inputbox(prompts, defaults, list, "Generate Stirrup 90°")
    ...
    def self.generate_system(opts)
      ...
      begin
        ...
        model.start_operation("Create Stirrup 90 System", true)
        ...
        comp_def_name = "Stirrup_90_#{opts[:type]}_#{opts[:width].to_mm.to_i}x#{opts[:depth].to_mm.to_i}_C#{cover_mm}_H12DB_STD"
    ...
    def self.draw_stirrup_geometry(entities, opts)
      ...
      # ระยะ detailing ของงอ 90 องศา = 12db; ลดรัศมีดัดออกจากช่วงเส้นตรงเพื่อไม่ให้ยื่นเกินในโมเดล
      hook_detail_len = 12 * dia
      hook_len = [hook_detail_len - bend_r, dia].max
      ...
      # 1. START HOOK 90 องศา (ปลายขาวิ่งเข้าด้านในตามแกน Y ลบ)
      start_arc_pt = get_circle_point(c_tl, bend_r, 180.degrees)
      vec_tail_start = Geom::Vector3d.new(0, -1, 0).normalize
      start_tip = start_arc_pt.offset(vec_tail_start, hook_len)
      
      all_points << start_tip
      all_points.concat(get_arc_points(c_tl, bend_r, 180, 90)) # CW
      ...
      # 3. END HOOK 90 องศา (ปลายขาวิ่งเข้าด้านในตามแกน X บวก + ยกหลบ)
      end_hook_points = []
      end_hook_points.concat(get_arc_points(c_tl, bend_r, 180, 90)) # Left -> Top
      
      end_arc_pt = get_circle_point(c_tl, bend_r, 90.degrees)
      vec_tail_end = Geom::Vector3d.new(1, 0, 0).normalize
    ...
    def self.show_report(opts, count, unit_len, total_len, weight)
      ...
      dlg = UI::HtmlDialog.new(
        {
          :dialog_title => "Stirrup 90 Estimation Report",
          :preferences_key => "com.gorebar.report90",
        ...
        <body>
          <div class="card">
            ...
            <table>
              ...
              <tr><td class="label">Hook Type</td><td class="val">90&deg; (12db standard)</td></tr>
...
GoRebar::StirrupSystem90.show_dialog
