...
module GoRebar
  module StirrupSystem180
    ...
    def self.show_dialog
      ...
      inputs = UI.inputbox(prompts, defaults, list, "Generate Stirrup 180°")
    ...
    def self.generate_system(opts)
      ...
      begin
        ...
        model.start_operation("Create Stirrup 180 System", true)
        ...
        comp_def_name = "Stirrup_180_#{opts[:type]}_#{opts[:width].to_mm.to_i}x#{opts[:depth].to_mm.to_i}_C#{cover_mm}_DSTD_H4DB60"
    ...
    def self.draw_stirrup_geometry(entities, opts)
      ...
      inside_bend_d = standard_bend_inside_diameter(dia)
      bend_r = (inside_bend_d / 2.0) + radius
      z_clash_offset = dia 
      
      # สูตรปลายขาตรงหลังโค้ง 180 องศา: อย่างน้อย 4db แต่ไม่น้อยกว่า 60 มม.
      hook_len = [4 * dia, 60.mm].max
      ...
      # 1. START HOOK 180 องศา (ครึ่งวงกลม + ปลายขาวิ่งเข้าด้านในตามแกน X บวก)
      start_arc_pt = get_circle_point(c_tl, bend_r, 270.degrees)
      vec_tail_start = Geom::Vector3d.new(1, 0, 0).normalize
      start_tip = start_arc_pt.offset(vec_tail_start, hook_len)
      
      all_points << start_tip
      all_points.concat(get_arc_points(c_tl, bend_r, 270, 90)) # CW
      ...
      # 3. END HOOK 180 องศา (ครึ่งวงกลม + ปลายขาวิ่งเข้าด้านในตามแกน Y ลบ + ยกหลบ)
      end_hook_points = []
      end_hook_points.concat(get_arc_points(c_tl, bend_r, 180, 0)) # Left -> Right
      
      end_arc_pt = get_circle_point(c_tl, bend_r, 0.degrees)
      vec_tail_end = Geom::Vector3d.new(0, -1, 0).normalize
    ...
    def self.show_report(opts, count, unit_len, total_len, weight)
      ...
      dlg = UI::HtmlDialog.new(
        {
          :dialog_title => "Stirrup 180 Estimation Report",
          :preferences_key => "com.gorebar.report180",
        ...
        <body>
          <div class="card">
            ...
            <table>
              ...
              <tr><td class="label">Hook Type</td><td class="val">180&deg; (4db, min 60mm)</td></tr>
    ...
    def self.validate_options!(opts)
      ...
      radius = opts[:dia] / 2.0
      bend_r = (standard_bend_inside_diameter(opts[:dia]) / 2.0) + radius
    ...
    def self.standard_bend_inside_diameter(dia)
      return 6 * dia if dia <= 25.mm
      return 8 * dia if dia <= 36.mm
...
GoRebar::StirrupSystem180.show_dialog
